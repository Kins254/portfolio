...
sapply(low_birth_weight_final_2,is.numeric)# checking the numeric variables for the test

# Select the numeric columns for which you want to perform the Shapiro-Wilk test
selected_columns <- c("age","low","ftv","ptl","lwt","smoke","ht","ui")

# Perform Shapiro-Wilk test on each selected variable
shapiro_results <- lapply(low_birth_weight_final_2[selected_columns], function(x) shapiro.test(x))

# Display the results
names(shapiro_results) <- selected_columns
shapiro_results
#with our results,we are going to use t test for normally distributed variables like age,ftt and lwt, and use mann whitney for the others


# variance testing for 2 sample and welch t test
